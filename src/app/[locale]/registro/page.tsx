...
export default function RegistroPage() {
  ...
  async function handleRegistro(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      options: {
        data: { nombre: form.nombre, negocio: form.negocio, telefono: form.telefono }
      }
    })
    if (error) {
...
cat > src/app/\[locale\]/registro/page.tsx << 'ENDOFFILE'
"use client"
import { useState } from 'react'
import Image from 'next/image'
import Link from 'next/link'
      ...
      <div className="w-full max-w-md">
        ...
        <div className="card">
          ...
          <p className="text-center text-sm text-brand-gray-mid mt-6">
            Ya tienes cuenta?{' '}
            <Link href="/es/login" className="text-brand-orange font-semibold hover:underline">
              Inicia sesion
            </Link>
          </p>
        </div>
