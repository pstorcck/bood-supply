      ...
      <div className={`flex-1 ${sidebarOpen?'ml-56':'ml-0'} min-h-screen flex flex-col transition-all duration-300`}>
            ...
            {showFormUsuario && (
              <div className="card border-2 border-brand-orange/30 mb-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><label className="block text-xs font-medium text-brand-gray-dark mb-1">Estado *</label><select value={formUsuario.estado} onChange={e=>setFormUsuario({...formUsuario,estado:e.target.value})} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-orange bg-white">{US_STATES.map(([code,name])=><option key={code} value={code}>{name}</option>)}</select><p className="text-[10px] text-brand-gray-mid mt-0.5">Clientes fuera de IL no pagan tax de quimicos</p></div>
                  ...
                  <div><label className="block text-xs font-medium text-brand-gray-dark mb-1">Vendedor Asignado</label><select value={formUsuario.vendedor_id} onChange={e=>setFormUsuario({...formUsuario,vendedor_id:e.target.value})} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-orange bg-white"><option value="">House</option>{clientes.filter(c=>c.role==='vendedor').map(v=><option key={v.id} value={v.id}>{v.nombre||v.email}</option>)}</select></div>
                ...
                <div key={c.id} className={`card border-l-4 ${c.aprobado?'border-l-green-400':'border-l-yellow-400'}`}>
                  ...
                  {editando?(
                    <div className="grid grid-cols-2 gap-3 mt-3 border-t pt-3">
                      ...
                      <div><label className="block text-xs font-medium text-brand-gray-dark mb-1">Estado *</label><select value={formCliente.estado||'IL'} onChange={e=>setFormCliente({...formCliente,estado:e.target.value})} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-orange bg-white">{US_STATES.map(([code,name])=><option key={code} value={code}>{name}</option>)}</select></div>
        ...
        {tab === 'productos' && (
          <>
            ...
            <div className="flex items-center justify-between mb-6"><p className="text-brand-gray-mid text-sm">{productos.length} productos · {productos.filter(p=>p.activo).length} activos</p><div className="flex items-center gap-2"><a href="/es/lista-precios" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-brand-navy text-white font-body font-semibold px-6 py-3 rounded-button hover:bg-brand-navy/80 transition-all duration-200"><Receipt size={18}/> Generar Lista de Precios</a><button onClick={()=>setShowFormProducto(!showFormProducto)} className="btn-primary flex items-center gap-2"><Plus size={18}/> Agregar Producto</button></div></div>
            {showFormProducto&&(
